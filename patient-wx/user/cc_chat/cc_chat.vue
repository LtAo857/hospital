<template>
	<view class="page">
		<view class="hero-card">
			<view class="hero-title">CC Agent挂号助手</view>
			<view class="hero-desc">第三套架构：先规划，再按步骤串行执行，把查询推进到可确认结果。</view>
		</view>
		...
		<scroll-view class="message-list" scroll-y :scroll-into-view="scrollAnchor">
			...
			<view v-if="agent.steps && agent.steps.length > 0" class="section-card">
				...
				<view class="step-list">
					<view v-for="step in agent.steps" :key="step.key" class="step-item">
						<text class="step-label">{{ step.label }}</text>
						<text :class="step.status === 'completed' ? 'step-status success' : step.status === 'in_progress' ? 'step-status active' : 'step-status'">
			...
			<view v-if="agent.cards && agent.cards.length > 0" class="section-card">
				...
				<view class="card-list">
					<view
						v-for="(card, index) in agent.cards"
						:key="index"
						class="action-card"
						@tap="handleCard(card)"
					>
			...
			<view v-if="agent.toolLogs && agent.toolLogs.length > 0" class="section-card slim">
				...
				<view v-for="(log, index) in agent.toolLogs" :key="index" class="tool-log">
	...
	methods: {
		...
		appendAssistantMessageStreaming(text) {
			...
			this.streamTimer = setInterval(function() {
				if (cursor >= chars.length) {
					that.clearStreamTimer();
					return;
				}
				that.messages[messageIndex].content += chars[cursor];
				cursor += 1;
				that.scrollToBottom();
			}, 20);
		},
		...
		scrollToBottom() {
			...
			this.$nextTick(function() {
				if (that.messages.length > 0) {
					that.scrollAnchor = 'msg-' + (that.messages.length - 1);
				} else {
					that.scrollAnchor = 'anchor-bottom';
				}
			});
		},
		submitText() {
			...
			this.chat({
				message: text,
				currentPage: '/user/cc_chat/cc_chat'
			});
		},
		...
		handleCard(card) {
			...
			if (card.action === 'navigate') {
				url = card.payload && card.payload.url ? card.payload.url : '';
				if (!url) {
					return;
				}
			...
			if (card.action === 'create_registration') {
				uni.showModal({
					title: '确认挂号',
					content: card.description || 'CC Agent 已准备好提交挂号，是否继续？',
		...
		chat(data) {
			...
			this.ajax(
				this.api.agentCcChat,
				'POST',
				request,
				resp => {
					let result = (resp.data && resp.data.result) || {};
					...
					this.appendAssistantMessageStreaming(result.reply || 'CC Agent 暂时没有返回内容。');
				},
				false
			);
		}
