package com.example.hospital.patient.wx.api.agent.react.tool;
...
@Component
public class TraditionalAgentToolRegistry {
    ...
    @PostConstruct
    public void init() {
        register("get_user_card_status", "Check whether the current logged-in user has a patient card and return brief details when available.", false);
        register("list_departments", "List outpatient departments. Optional input: deptName.", false);
        register("list_sub_departments", "List clinic rooms under a department. Input: deptId or deptName.", false);
        register("list_register_dates", "List the next 7 days of registration availability for a clinic room. Input: deptSubId or deptSubName.", false);
        register("list_doctors_in_day", "List doctors available in a clinic room on a date. Input: deptSubId or deptSubName, plus date.", false);
        register("list_schedule_slots", "List bookable schedule slots for a doctor on a date. Input: doctorId or doctorName, plus date.", false);
        register("list_messages", "List latest user messages and unread count.", false);
        register("create_registration", "Create a registration order. Requires complete slot information and confirmed=true.", true);
    }
    ...
    public List<Map<String, Object>> describeTools() {
        ...
        for (ToolDefinition definition : definitions.values()) {
            ...
            item.put("writeOperation", definition.writeOperation);
            result.add(item);
        }
        return result;
    }
    ...
    public TraditionalAgentToolResult execute(String toolName,
        ...
        switch (toolName) {
            ...
            case "list_sub_departments":
                return listSubDepartments(toolInput, memory);
            case "list_register_dates":
                return listRegisterDates(toolInput, memory);
            case "list_doctors_in_day":
                return listDoctorsInDay(toolInput, memory);
            case "list_schedule_slots":
                return listScheduleSlots(toolInput, userId, memory);
            case "list_messages":
                return listMessages(userId);
            case "create_registration":
                return createRegistration(toolInput, userId, memory);
            default:
                TraditionalAgentToolResult result = new TraditionalAgentToolResult();
    ...
    private TraditionalAgentToolResult getUserCardStatus(Integer userId) {
        TraditionalAgentToolResult result = new TraditionalAgentToolResult();
        if (userId == null) {
    ...
    private TraditionalAgentToolResult listDepartments(Map<String, Object> toolInput) {
        TraditionalAgentToolResult result = new TraditionalAgentToolResult();
        ArrayList<HashMap> list = medicalDeptAgentTools.searchDepartments(null, true);
        String deptName = firstString(toolInput == null ? null : toolInput.get("deptName"));
    ...
    private TraditionalAgentToolResult listSubDepartments(Map<String, Object> toolInput, Map<String, Object> memory) {
        TraditionalAgentToolResult result = new TraditionalAgentToolResult();
        Integer deptId = firstInt(value(toolInput, "deptId"), memory.get("deptId"));
        String deptName = firstString(value(toolInput, "deptName"), memory.get("deptName"));
        ...
        String deptSubName = firstString(value(toolInput, "deptSubName"), memory.get("deptSubName"));
    ...
    private TraditionalAgentToolResult listRegisterDates(Map<String, Object> toolInput, Map<String, Object> memory) {
        TraditionalAgentToolResult result = new TraditionalAgentToolResult();
        Integer deptSubId = firstInt(value(toolInput, "deptSubId"), memory.get("deptSubId"));
        String deptSubName = firstString(value(toolInput, "deptSubName"), memory.get("deptSubName"));
        if (deptSubId == null && StringUtils.hasText(deptSubName) && memory.get("deptId") != null) {
            ArrayList<HashMap> list = medicalDeptAgentTools.searchSubDepartments(intValue(memory.get("deptId")));
            HashMap matched = matchByName(list, deptSubName, "name");
            if (matched != null) {
                deptSubId = intValue(matched.get("id"));
                deptSubName = stringValue(matched.get("name"), deptSubName);
            }
        }
        if (deptSubId == null) {
            result.setSuccess(false);
            result.setSummary("Clinic room is required before querying registration dates");
        ...
        String selectedDate = firstString(value(toolInput, "date"), memory.get("date"));
        HashMap matchedDate = matchByName(dates, selectedDate, "date");
        if (matchedDate == null && StringUtils.hasText(selectedDate)) {
    ...
    private TraditionalAgentToolResult listDoctorsInDay(Map<String, Object> toolInput, Map<String, Object> memory) {
        TraditionalAgentToolResult result = new TraditionalAgentToolResult();
        Integer deptSubId = firstInt(value(toolInput, "deptSubId"), memory.get("deptSubId"));
        String date = firstString(value(toolInput, "date"), memory.get("date"));
        String doctorName = firstString(value(toolInput, "doctorName"), memory.get("doctorName"));
    ...
    private TraditionalAgentToolResult listScheduleSlots(Map<String, Object> toolInput, Integer userId, Map<String, Object> memory) {
        TraditionalAgentToolResult result = new TraditionalAgentToolResult();
        Integer doctorId = firstInt(value(toolInput, "doctorId"), memory.get("doctorId"));
        String doctorName = firstString(value(toolInput, "doctorName"), memory.get("doctorName"));
        String date = firstString(value(toolInput, "date"), memory.get("date"));
        ...
        for (HashMap item : schedules) {
            ...
            slot.put("workPlanId", item.get("workPlanId"));
            slot.put("scheduleId", item.get("scheduleId"));
            slot.put("slot", item.get("slot"));
            slot.put("slotLabel", slotLabel(intValue(item.get("slot"))));
            slot.put("maximum", item.get("maximum"));
            slot.put("num", item.get("num"));
            slot.put("amount", doctor == null ? null : doctor.get("price"));
            slots.add(slot);
        }
        result.getMemoryUpdates().put("doctorId", doctorId);
        result.getMemoryUpdates().put("date", date);
        if (StringUtils.hasText(doctorName)) {
            result.getMemoryUpdates().put("doctorName", doctorName);
        } else if (doctor != null && doctor.get("name") != null) {
            result.getMemoryUpdates().put("doctorName", doctor.get("name"));
        }
    ...
    private TraditionalAgentToolResult listMessages(Integer userId) {
        TraditionalAgentToolResult result = new TraditionalAgentToolResult();
        if (userId == null) {
            result.setSummary("User is not logged in");
            result.setObservation(Collections.singletonMap("requiresLogin", true));
    ...
    private TraditionalAgentToolResult createRegistration(Map<String, Object> toolInput, Integer userId, Map<String, Object> memory) {
        TraditionalAgentToolResult result = new TraditionalAgentToolResult();
        if (userId == null) {
            result.setSuccess(false);
            result.setSummary("User is not logged in");
        ...
        mergeFromMemory(payload, memory, "workPlanId", "scheduleId", "doctorId", "doctorName", "deptSubId", "deptSubName", "date", "slot", "amount");
    ...
    private void mergeFromMemory(Map<String, Object> payload, Map<String, Object> memory, String... keys) {
        for (String key : keys) {
            if (!payload.containsKey(key) && memory.containsKey(key) && memory.get(key) != null) {
