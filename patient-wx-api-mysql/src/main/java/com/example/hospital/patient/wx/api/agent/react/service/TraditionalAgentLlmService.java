...
@Service
public class TraditionalAgentLlmService {
    ...
    private JSONObject buildRequestBody(AgentChatRequest request,
        ...
        messages.add(new JSONObject().set("role", "system").set("content", TraditionalAgentPromptCatalog.getSystemPrompt()));
    ...
    private String buildUserPrompt(AgentChatRequest request,
        ...
        builder.append("1. Use one tool at a time.\n");
        builder.append("2. Prefer querying facts before you finish.\n");
        builder.append("3. If the user asks for messages, use list_messages.\n");
        builder.append("4. If the user asks for identity, real-name info, or patient card, use get_user_card_status.\n");
        ...
        builder.append("9. Your finalAnswer must be grounded in the latest tool observation. Do not invent doctor names, slot availability, or schedule conclusions.\n");
        builder.append("10. Treat toolInput entity fields as grounded data only. Do not invent deptName, deptSubName, doctorName, doctorId, workPlanId, or scheduleId. If those are missing, choose an earlier discovery tool.\n");
    ...
    private TraditionalAgentModelDecision buildFallbackDecision(AgentChatRequest request, Map<String, Object> memory) {
        ...
        decision.setFinalAnswer("我是第二套传统 Agent 架构，目前可以帮助你查询科室、诊室、医生、时段、消息和就诊卡状态，也可以在确认后提交挂号。");
        return decision;
    }
