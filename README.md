...
当前项目与大规模生产环境 Agent 的核心分层思路一致——Python 管理解、Java 管执行——差异源于规模和场景复杂度：
...
| **安全审核** | 生成回复后过安全审核节点 | 高危意图入口拦截 | 对外客户可见内容需审核；挂号内部流转，NLU 入口阻断即可 |
| **核心原则** | Python 管思维（prompt + LLM + 流程编排），Java 管执行（查库 + 退款 + 事务） | 完全一致 | 两者都遵循"LLM 只决策不执行"的架构铁律 |

当前项目受限于单机规模与挂号场景的确定性，未落地大厂做法。但每一项的触发条件、技术路径和接入节点都已明确——量级到了、复杂到了、延迟要求到了，即加即用。
